...
export function BarAndLineChart(
  ...
  if (xDomain === undefined) xDomain = data.map((d) => d.year); //d3.extent(X);
  ...
  const xScale = xType(xDomain, xRange).padding(0.1);
  ...
  const bar = svg
    ...
    .attr("fill", "steelblue")
    .selectAll("rect")
    .data(data.map((d) => ({ year: d.year, count: d.count })))
    .join("rect")
  ...
  function pointermoved(event) {
    ...
    dot.attr(
      "transform",
      `translate(${xScale(X[i]) + xScale.bandwidth() / 2},${yScale(Y[i])})`
    );
